...
class SentimentScore():
    ...
    def __init__(self, workingPath):
        ...
        self.excelFile = '../Data/Interim/ProcessedTickerFinal_2019-12-18.xlsx'
        self.failedFileCsv = '../Data/Log/FailedFileList_2019-12-18.csv'
        self.fileList = []
        self.wpList = []
        self.pdfTextList = []
        self.periodList = []
...
#         self.df['AnnouncementPeriod'] = self.df['AnnouncementDate'].map(lambda x:monthDict[pd.to_datetime(x).month] if                                                                                              pd.notnull(pd.to_datetime(x)) else '')
#         self.df['AnnouncementYear'] = self.df['AnnouncementDate'].map(lambda x:str(pd.to_datetime(x).year) if                                                                                                                pd.notnull(pd.to_datetime(x)) else '')
#         self.df['Period'] = self.df.apply(lambda x:x['Period'] if x['AnnouncementPeriod'] == '' else x['AnnouncementPeriod'], axis = 1)
#         self.df['Year'] = self.df.apply(lambda x:x['Year'] if x['AnnouncementYear'] == '' else x['AnnouncementYear'], axis = 1)
    ...
    def calculateScore(self):
        ...
        self.dfFinal = (self.df.filter(['FullStockName', 'Period', 'Year', 'ContentType', 'SentimentScore_Magnify'])
    ...
    def writeResult(self):
        ...
        self.dfFinal.to_excel(self.excelFile, index = False, encoding = 'utf-8')
        return self.dfFinal
